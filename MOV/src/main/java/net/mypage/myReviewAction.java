...
public class myReviewAction implements Action {
	...
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response)
		...
		if (request.getParameter("search_word") == null
			...
			// 총 리스트 수를 받아옵니다.
			listcount = mdao.getmyReviewCount(email);
			boardlist = mdao.getmyReview(email, page, limit);
		} else { // 검색을 클릭한 경우
			search_word = request.getParameter("search_word");
			listcount = mdao.getmyReviewCount(email, search_word);
			boardlist = mdao.getmyReview(email, search_word , page, limit);
		}
